...
output_file = "~/src/pseudogenes/pseudogenes_lines.txt"

# Create output file
cmd = f'~/src/pseudogenes/pseudogenes_lines.txt'
...
def process_chromosome(chromosome):
    # Build the command
    cmd = f'gunzip -c {gff3_file} | grep -v "#" | awk \'$1 == "chr{chromosome}" && $3 == "gene" && $9 ~ "pseudogene"\' | wc -l'
