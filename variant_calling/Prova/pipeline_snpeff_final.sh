...
# Annotate VCF with SnpEff (snpEff needs to be installed and configured with GRCh38.99)
echo "Annotating SNPs and INDELs with SnpEff"
...
# Filter and prioritize variants based on Impact, CADD, ClinVar, and gnomAD frequency using SnpSift

# For SNPs
echo "Prioritizing SNPs based on Impact, CADD PHRED, ClinVar Pathogenicity, and gnomAD Frequency"
SnpSift filter "((ANN[*].IMPACT = 'HIGH') | (ANN[*].IMPACT = 'MODERATE')) & (CADD_PHRED >= 20) & ((CLNSIG = 'Pathogenic') | \
(CLNSIG = 'Likely_pathogenic')) & (AF[0] < 0.01)" ${results}/analysis-ready-snps-annotated.vcf > ${results}/priority_snps.vcf

# For INDELs
echo "Prioritizing INDELs based on Impact, CADD PHRED, ClinVar Pathogenicity, and gnomAD Frequency"
SnpSift filter "((ANN[*].IMPACT = 'HIGH') | (ANN[*].IMPACT = 'MODERATE')) & (CADD_PHRED >= 20) & ((CLNSIG = 'Pathogenic') | \
(CLNSIG = 'Likely_pathogenic')) & (AF[0] < 0.01)" ${results}/analysis-ready-indels-annotated.vcf > ${results}/priority_indels.vcf

# Generate tab-delimited files for easier review
echo "Converting prioritized variants to tab-delimited format"
vcf2csv ${results}/priority_snps.vcf > ${results}/priority_snps.csv
vcf2csv ${results}/priority_indels.vcf > ${results}/priority_indels.csv
